...
class AGENT(object):
    ...
    def __str__(self):
        ret = "({ACTI}A#{AID}, T#{TID}), tz({TMIN},{TMAX})".format(AID=self.agent_id, TID=(self.task_id if not self.task_id is None else "--"), ACTI=("o" if self.is_activated else "x" ), TMIN=self.T_zone[0], TMAX=self.T_zone[1] )
        return ret
